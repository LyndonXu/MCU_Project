...
void RedressPushRodLimit (void)
{
	...
	ChangeLedState(GET_X(_Led_Play_Minus_Plus), GET_Y(_Led_Play_Minus_Plus), true);
	...
	while(1)
	{
		...
		{
			if (pKey->u8KeyValue == _Key_Play_IN)
			{
				u32MsgSentTime = g_u32SysTickCnt;
				if (pKey->u8KeyState == KEY_DOWN)
				{
					ChangeLedState(GET_X(_Led_Play_IN), GET_Y(_Led_Play_IN), true);
				}
				else if (pKey->u8KeyState == KEY_UP)
				{
					ChangeLedState(GET_X(_Led_Play_IN), GET_Y(_Led_Play_IN), false);
		...
		{
			if (pKey->u8KeyValue == _Key_Play_OUT)
			{
				u32MsgSentTime = g_u32SysTickCnt;
				if (pKey->u8KeyState == KEY_DOWN)
				{
					ChangeLedState(GET_X(_Led_Play_OUT), GET_Y(_Led_Play_OUT), true);
				}
				else if (pKey->u8KeyState == KEY_UP)
				{
					ChangeLedState(GET_X(_Led_Play_OUT), GET_Y(_Led_Play_OUT), false);
	...
	if (PushRodGetTheRedressLimit(u16UpLimit, u16DownLimit))
	{
		if (WriteSaveData())
		{
			ChangeLedState(GET_X(_Led_Play_Minus_Plus), GET_Y(_Led_Play_Minus_Plus), false);
			u32MsgSentTime = g_u32SysTickCnt;
...

	{
		...
		{
			boBlink = !boBlink;
			ChangeLedState(GET_X(_Led_Play_Minus_Plus), GET_Y(_Led_Play_Minus_Plus), boBlink);
			u32MsgSentTime = g_u32SysTickCnt;
