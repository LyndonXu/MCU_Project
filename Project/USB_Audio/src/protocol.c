...
#define APP_VERSOIN	"YA_USB_AUDIO_181111"
	...
	if (pMsg[_YNA_Mix] == 0x06)
	{
		...
		switch(pMsg[_YNA_Cmd])
		{
			...
			case 0x43:
			{
				...
				SaveMemoryFromDevice((g_stSave.stMemory + u8Index));
			...
			case 0x44:
			{
				...
				LoadMemoryToDevice((g_stSave.stMemory + u8Index));
				break;
			}
			case 0x46:
			{	
				...
				{
					bool boState[FANTASY_POWER_CTRL] = {0};
					StMixAudioPhantomPowerMode *pMode = malloc(FANTASY_POWER_CTRL * 
						sizeof(StMixAudioPhantomPowerMode));
					boNeedCopy = false;
					if (pMode != NULL)
					{
						s32 i;
						GetAllFantasyPowerState(boState);
						...
						pEcho = YNAMakeAnArrayVarialbleCmd(0x0646, pMode, 
							FANTASY_POWER_CTRL, sizeof(StMixAudioVolume), &u32EchoLength);
						free(pMode);
					}
				}
				else
			...
			case 0x50:
			{
				...
				else if (pMsg[_YNA_Data1] == 0x01)
				{
					...
					u8EchoBase[_YNA_Data2] = WM8776GetAINChannelEnableState();
					u8EchoBase[_YNA_Data3] = WM8776GetOutputChannelEnableState();
				}
				else
				{
					u8EchoBase[_YNA_Cmd] = 0x50;
					u8EchoBase[_YNA_Data2] = I2CWrite(pMsg[_YNA_Data1], pMsg[_YNA_Data2], pMsg[_YNA_Data3]);
				}
